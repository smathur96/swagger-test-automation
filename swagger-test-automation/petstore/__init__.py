...
class BaseClass:
    ...
    def query(
        ...
        **kwargs,
        ...
        logger.info(
            "\n*************\nREQUEST MADE: execution_id: {}, method: {}, path: {}, data: {}\n".format(
                log.execution_id,
                method,
                url,
                str("Get request") if method == "GET" else data,
            )
        )
        ...
        if method == "GET":
            ...
            try:
                ...
                logger.info(f"GET {url} response: {response}")
                return response
...
class Pet(BaseClass):
    def __init__(self, base_url, api_key, log: Log) -> None:
